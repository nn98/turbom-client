        ...
        <div className="mx-auto max-w-lg px-4 py-24 text-center">
          {notFound ? (
            ...
            <>
              ...
              <Button variant="outline" className="mt-6" onClick={() => unitQuery.refetch()}>
                다시 시도
              </Button>
            </>
          )}
...
function ReportHeader({
  ...
  return (
    <Card
      className={
        "relative overflow-hidden rounded-[2rem] border-border/70 bg-surface p-6 shadow-elevated before:absolute before:inset-y-0 before:left-0 before:w-1 sm:p-8 " +
        accentClass
      }
    >
      <div>
...
function NarrativeCard({ lines }: { lines: string[] }) {
  return (
    <Card className="rounded-[2rem] border-border/70 bg-surface p-6 shadow-card sm:p-8">
...
function DistrictAnalysis({ district }: { district: UnitAnalysis["district"] }) {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1.4fr_1fr]">
      <Card className="rounded-[2rem] border-border/70 bg-surface p-6 shadow-card">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  type="button"
                  variant="outline"
                  className="h-8 rounded-xl border-border/70 px-3 text-xs text-muted-foreground"
                >
...
function RiskCard({ level, label }: { level: RiskLevel; label: string }) {
  ...
  return (
    <Card className="rounded-[2rem] border-border/70 bg-surface p-8 shadow-card">
...
function TimelineCard({ timeline }: { timeline: Tenancy[] }) {
  ...
  return (
    <div className="space-y-6">
      <Card className="rounded-[2rem] border-border/70 bg-surface p-4 shadow-card sm:p-6">
...
function ReportCta() {
  return (
    <div className="mt-20 overflow-hidden rounded-[2.5rem] bg-navy p-10 text-navy-foreground sm:p-14">
      <div className="flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-center">
        ...
        <Button asChild size="lg" className="bg-background text-navy hover:bg-background/90">
          <Link to="/search">
            새로운 자리 분석하기
