...
const pinHtml = (label: string, active?: boolean) => `
  <div style="transform:translate(-50%,-100%);display:flex;flex-direction:column;align-items:center;gap:6px;">
    ${
      active
        ? `<div style="padding:3px 8px;border:1px solid var(--color-border);border-radius:9999px;background:var(--color-surface);color:var(--color-navy);font-size:11px;font-weight:700;line-height:1;white-space:nowrap;box-shadow:0 8px 20px -8px oklch(0.22 0.02 255 / 0.25);">
      ${escapeHtml(label)}
    </div>`
        : ""
    }
